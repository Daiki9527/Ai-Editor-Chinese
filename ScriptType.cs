...
namespace AIEdit
{
	...
	public class ActionTypeList : IActionType, IComparable
	{
		...
		public string ParamToString(int param)
		{
			...
			return entry != null ? entry.ToString() : "<错误>";
		}
	...
	public class ScriptAction
	{
		...
		private static string[] offsetsDesc = { "最小威胁", "最大威胁", "最近威胁", "最远威胁" };
	...
	public class ScriptType : IAIObject, IEnumerable<ScriptAction>, IParamListEntry
	{
			...
			Logger logger)
		{
			...
			try
			{
				foreach(DictionaryEntry entry in section)
				{
					...
					else
					{
						...
						if (split.Length < 2)
						{
							logger.Add("ScriptType " + id + ": 公式不正确，如: <引索>=<行动>,<数值>");
						}
						else
						{
			...
			{
				string msg = "错误出现在 ScriptType: [" + id + "]" + "\n请确认它的书写. 应用即将关闭.";
